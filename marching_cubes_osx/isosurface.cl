void AddTriangle(__global float *tl, int triangleIdx, float X1, float Y1, float Z1, float X2, float Y2, float Z2, float X3, float Y3, float Z3)
{

	tl[9*triangleIdx+0] = X1;
	tl[9*triangleIdx+1] = Y1;
	tl[9*triangleIdx+2] = Z1;
	tl[9*triangleIdx+3] = X2;
	tl[9*triangleIdx+4] = Y2;
	tl[9*triangleIdx+5] = Z2;
	tl[9*triangleIdx+6] = X3;
	tl[9*triangleIdx+7] = Y3;
	tl[9*triangleIdx+8] = Z3;
};
...
__kernel void CalculatePrefix(
		__global float const *X, 
		__global float const *Y,
		__global float const *Z,
		__global float const *F,
		__global int const *dims, 
		__global int *tri_occupied,
		float iso_value) 
{
	...
	if (cell_id > nCells) return;

	// get Field values
	int idx[3];
	idx[0] = cell_id%(dims[0]-1);
	idx[1] = (cell_id/(dims[0]-1))%(dims[1]-1);
	idx[2] = cell_id/((dims[0]-1)*(dims[1]-1));
	...
	int icase = 0;
	if (f_val[0] > iso_value) icase += 1;
	if (f_val[1] > iso_value) icase += 2;
	if (f_val[2] > iso_value) icase += 4;
	if (f_val[3] > iso_value) icase += 8;
	if (f_val[4] > iso_value) icase += 16;
	if (f_val[5] > iso_value) icase += 32;
	if (f_val[6] > iso_value) icase += 64;
	if (f_val[7] > iso_value) icase += 128;
		...
		float iso_value) 
{
	...
	int nCells = (dims[0]-1)*(dims[1]-1)*(dims[2]-1);
	if (cell_id > nCells) return;
	if (tri_occupied[cell_id] == 0) return;

	int edge_to_vertex[12][2]  =
	{ {0,1},  {1,3},  {2,3},  {0,2},
		{4,5},  {5,7},  {6,7},  {4,6},
		{0,4},  {1,5},  {2,6},  {3,7} };

	int edge_to_bcube[12][6]  =
